...
public class BodyDisplay : MonoBehaviour, IBodyDisplay
{
    ...
    public void UpdateBodyPart(SO_DB.ItemData item)
    {
        ...
        /*unequips the current database bodypart and equips the new bodypart (this is useful when you change bodyparts from the inventory)*/
    ...
    private void EquipNewItem(SO_DB.ItemData item)
    {
        foreach (SO_DB.ItemData i in db.playerData.items)
        {
            /* find the equiped item and unequip it*/
            if(i.equip.bodyType == item.equip.bodyType && i.equiped)
            {
                i.equiped = false;
            }
